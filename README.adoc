...
=== apps vs kernels vs netapps

Everything under netapps is a net version of something under app, e.g.

* `pkgs/kernels/dedup/`
* `pkgs/netapps/netdedup`

Does that mean it is a version that can be parallelized over the network?
...
Two deps in particular are special PARSEC things and used widely:
...
Hooks are instrumentation hooks that get performance metrics out. They have several flavors for different environement, e.g. native vs magic simulator instructions.
