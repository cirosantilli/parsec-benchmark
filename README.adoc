...
PARSEC http://parsec.cs.princeton.edu/ 3.0-beta-20150206 ported to Ubuntu 25.10 and <<splash2>> ported to Buildroot 2017.08 cross compilation (ARM, MIPS, etc.). This repo intends to support all build types and benchmarks.
...
Install Ubuntu dependencies with apt-get and download the test data:
...
Build just one SPLASH2 benchmark, `splash2x.barnes`, which does a https://en.wikipedia.org/wiki/Barnes%E2%80%93Hut_simulation[Barnes-Hut N-body gravity hierarchical simulation]:
...
input.template
[PARSEC] Running 'time /home/ciro/bak/git/parsec-benchmark/ext/splash2x/apps/barnes/inst/amd64-linux.gcc/bin/run.sh 1 test':
[PARSEC] [---------- Beginning of output ----------]
Generating input file input_1...
Running /home/ciro/bak/git/parsec-benchmark/ext/splash2x/apps/barnes/inst/amd64-linux.gcc/bin/barnes 1 < input_1:

                Hack code: Plummer model

     nbody     dtime       eps       tol     dtout     tstop    fcells     NPROC
     65536   0.02500    0.0500      1.00     0.250     0.075      2.00         1

COMPUTESTART  = 1776666743854108
COMPUTEEND    = 1776666746784692
COMPUTETIME   =      2930584
TRACKTIME     =      1380332
PARTITIONTIME =         4064     0.00
TREEBUILDTIME =        38866     0.03
FORCECALCTIME =      1325106     0.96
RESTTIME      =        12296     0.01
...
In this case for example, 65536 is the number of simulated bodies of the N-body simulation.

Run a non-splash 2 benchmark:
...
Clean all builds without nuking the downloaded test input data with link:clean-build.sh[]:
...
=== parsecmgmt rebuilds after source changes

`parsecmgmt -a build` uses each package installation's `build-info` marker to decide whether that package is already available. We have modified the original build system so that the build step also checks whether package sources or PARSEC build configuration files are newer than that marker, and whether the package build directory still exists.

This means a direct rebuild after source edits works:

....
parsecmgmt -a build -p ferret
....

Cleaning the package first also forces a rebuild because `clean` removes the build directory:
...
* 2026-07-14: https://arxiv.org/html/2605.15222v2 PerfCodeBench: Benchmarking LLMs for System-Level High-Performance Code Optimization
