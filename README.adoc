...
This is known to be broken on Ubuntu 22.04 because `tbblib` is broken: https://github.com/cirosantilli/parsec-benchmark/issues/13
...
This is known to be broken on Ubuntu 22.04 because `splash2x` is broken: https://github.com/cirosantilli/parsec-benchmark/issues/13
...
We have to re-run `./get-inputs` again because the `git clen -xdf` removes the unpacked inputs that were placed under `pkgs/apps/ferret/inputs/`.
